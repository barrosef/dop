...
The base decisions: [ADR-0010](../../adr/0010-dynamic-workflow.md),
...
| `spec` | the spec with executable criteria | a document + an approval gate | ADR-0005; the criteria's syntax = P-8 |
...
| `test` | running the suites per subtype | aaa/e2e/integration tabs, results | green is required before the PR (ADR-0005) |
...
| `finalization` | PRs, entering the merge queue, closing | steps with idle/running/done/warn/error states | ADR-0005; a consolidated dossier |
...
**Access (ADR-0010 §6):** in an organization, flows are open within the account by default,
...
2. Every stage transition, artifact produced and gate decided is an **event** (ADR-0004); the
   ruler at the centre of the Chat is a projection.
3. A pending `human` gate becomes an item in the **attention box**.
4. Stages generate the artifacts in the knowledge storage (ADR-0006); the subagents' findings
   attach to the current stage.

## 5. The platform's default

A global resource from the catalogue (adoptable as a versioned copy — ADR-0009 §3):
...
| R-3 | The type vocabulary growing without control — a new type requires a platform decision (ADR-0010 §1) |
