...
| **Second factor** | The second step of a sign-in, the platform's and not the identity provider's (ADR-0020). Three kinds: `totp` (an authenticator app), `email` and `sms` |
...
| **Workflow** | A resource that composes the demand's cycle into typed stages, with artifacts and gates (ADR-0010). Inheritable along the chain platform ◁ account ◁ workspace ◁ project ◁ demand |
...
| **Techlead (the project orchestrator)** | The project's agent, activated with parallel demands: it detects cross-cutting situations, plans and prompts decisions in the attention box; it never pauses demands (ADR-0011) |
...
| **Runner** | Where a VERIFICATION runs: an ephemeral environment, separate from the sandbox, that pulls a COMMIT, builds from source and starts the application (ADR-0023). It starts from nothing, which is what makes its evidence about a clean environment and not about the agent's |
...
| **Root repository** | A project's own git repository, born with the project in the platform's git server, holding its knowledge as files (`rules/`, `index/`, `memory/`, `demand/<id>/`). Every sandbox of the project clones it at `/project`; the user may attach a remote of theirs as a mirror (ADR-0021) |
...
| **Artefact** | A file at `demand/<id>/<kind>.md` in the root repository — spec, plan, test plan, report — declared by a flow's stage (ADR-0010). Rendered bytes stay in the ObjectStore, referenced from the file |
...
deliberately does NOT do for a customer's application (ADR-0023) — the only
