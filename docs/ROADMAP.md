...
| P-16 | **REOPENED on 2026-09-03 — what was built is the wrong kind.** The owner's definition: ONE volume per PROJECT, mounted read-write by every sandbox of that project, **shared and collaborated** between agents, with no sends, no transports and no ConfigMaps — the storage IS the volume. What was delivered on 09-02/03 (a per-demand claim filled by a loader pod pushing a tar over stdin, read-only, rewritten at every resume) is a transport, and it is flagged as a serious warning in [`analysis/2026-09-03-project-knowledge-availability.md`](analysis/2026-09-03-project-knowledge-availability.md). What survives: the port constant `/project`, the library's tree and manifest, and the idea of guarantees — whose text flips (writable, persistent, per project, isolated between projects). Two ways, behind the same constant: an RWX volume per project, or the object store mounted (GCS FUSE on GKE). The local cluster refuses RWX today (P-39) | ADR-0009; ADR-0024; ADR-0001 |
...
| P-18 | **ROUND 2 — to be explained from scratch, at the owner's request.** **Authenticating the caller between the BFF and the core** — the core trusts the metadata's `x-actor-id`/`x-account-id` (ADR-0016). The NetworkPolicy makes the assumption hold, but that is not the same as authenticating. Sandboxes run agent code in the same cluster | dop-core + dop-infra; ADR-0016 |
...
| P-39 | **The local cluster has no ReadWriteMany.** Verified on 2026-09-03: `local-path` answers "NodePath only supports ReadWriteOnce and ReadWriteOncePod". A project volume mounted by several demand sandboxes at once IS RWX, so the expectation in P-16 cannot run on k3d as it stands — it needs an NFS provisioner in the cluster, or a storage-backed CSI driver (GCS FUSE has none for k3d). An infra task that comes BEFORE P-16's mechanism | dop-infra |
| P-40 | **Attribution on a shared knowledge volume.** ADR-0006's premise is telling apart what the agent did from what the human did. A plain shared filesystem loses it — a file changed, by whom? The cheapest known answer is a git-backed knowledge directory (a commit per write, author = the thread), which also gives history and rollback. It has to be decided BEFORE agents start writing, because retrofitting attribution onto files already written is not possible | **a discussion**; ADR-0006; touches P-16 |
