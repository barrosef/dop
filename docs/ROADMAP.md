...
| P-45 | **Sharing a development flow between accounts** — designed on 2026-09-05: [`2026-09-05-development-flow-sharing-design.md`](superpowers/specs/2026-09-05-development-flow-sharing-design.md). Publication under `@handle/slug@vN`, grant to a named account, adoption as a COPY with provenance recorded on both sides, and a revocation policy (`prospective` \| `drain` \| `terminate`) stamped on the grant. The agent proposes and cannot persist; a human confirms a diff. One renderer serves definition, progress and diff. **It partially reopens P-9**: sharing to a named account is in, the public catalogue stays out | dop-core + dop-app; designed, not built |
