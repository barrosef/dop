...
| P-14 | **Adaptador do `GitProvider`** — a porta existe no domínio de entrega (abrir PR, rebase, merge, detectar fila nativa) e devolve conflito como DADO, não erro. Falta o adaptador que fala GitHub/GitLab | dop-core; ADR-0007/0008 |
| P-15 | **Ligar o launcher** — `RegisterLauncher` é esqueleto. O RBAC de cluster já está escrito e propositalmente desligado em `dop-infra/k3s/services/dop-core/rbac-launcher.yaml` | dop-core + dop-infra |
...
| P-20 | **A borda ainda usa os contornos** — agora que o núcleo devolve procedência estruturada e `dropped`, o BFF deveria parar de fazer parsing da frase e de detectar campo por descritor. O contorno funciona, mas é dívida com prazo curto: quem ler o código vai copiar o parsing | dop-api |
| P-21 | **Caixa de atenção não chegou à borda** — o núcleo serve `AttentionService` (`ListAttention` + `WatchAttention`), e o BFF não expõe. É a fila que responde "onde eu sou necessário": sem ela na borda, o cockpit não tem como mostrá-la | dop-api |
| P-22 | **O BFF não pode ter segredo** — invariante nova, irmã de "o BFF não tem banco" (ADR-0016/0023). Precisa virar teste que falhe se alguém der `SecretStore` ou chave de provedor ao BFF; hoje é só disciplina | dop-api |
