...
| P-16 | **DECIDED on 2026-09-03: the project's knowledge is a GIT REPOSITORY, not a volume.** Instead of sharing a PVC, the sandboxes share a repo: every sandbox of the project clones it, agents write by committing, and sharing is push/pull. It dissolves P-39 (no ReadWriteMany needed — each sandbox has its own working copy) and answers P-40 (attribution is the commit's author; history and rollback come free). What survives from the earlier work: the tree and the manifest of `knowledge/library.go` become the repository's layout and its `README.md`. What is left to decide — see the two questions in the ADR to be written: WHERE the repository lives (recommended: a dedicated knowledge repo per project in the account's git provider, created through the `GitProvider` port) and HOW a sandbox authenticates to git, which is the same question P-38 has for the code repositories | ADR to write; ADR-0009; ADR-0006; ADR-0013 |
