...
| P-19 | **Lacunas do contrato do núcleo que a borda contornou** — `ListFindings` (existe no domínio, não no proto); `Contributors`/`Origins` estruturados em `EffectiveFlow` (a borda faz parsing de uma frase); consulta de evidência de verde (a tela do PR não mostra o pacote que a ADR-0007 exige); `dropped` em `ContextPackage` e `currency` em `Budget` | dop-core |
