...
| P-16 | **DECIDED — ADR-0028 (2026-09-03).** The project's knowledge is its ROOT REPOSITORY, born under the hood in a git server the platform runs; every sandbox of the project clones it at `/project`, agents write by committing (author = the thread), sharing is push/pull, every push is an event. The user may attach a remote of their own at any time: the platform's repository stays primary and the remote is a push MIRROR — no two-way sync in v1. The sandbox holds one credential: a per-demand token that opens exactly that repository. What is left is the BUILD: the git server in dop-infra, the `ProjectRepository` port with two adapters and a contract suite, `git` in the devbox, the clone at provisioning, and the removal of the loader mechanism | ✅ decided; build pending; ADR-0028 |
