...
| P-37 | **What is left of US-5.3** — the role is editable on the members screen (a select on the row, with the last-owner invariant now in the domain), but **unlinking** a member and editing **resource grants** still have no screen. Unlinking is the one that carries a rule nobody has written yet: what happens to what that person created in the account | dop-core + dop-app |
