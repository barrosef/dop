...
| P-25 | **⭐ Claude Code hosted in the sandbox — REPOSITIONED on 2026-09-03: it is not the second front, it is the door.** The sandbox runs the binary and the user signs in with their OWN subscription, through Anthropic's flow: it solves the "paying twice" problem without intermediating a credential. **Why the order changed:** the terms were re-read at the source and they separate the two paths by AUDIENCE, not by execution model — BYOK reaches only whoever has an **API account** (Console, pay-as-you-go), because storing a Pro/Max credential in our vault is exactly the "collect, store or intermediate Claude.ai credentials" that is forbidden; **Pro and Max are reachable ONLY here**, precisely because we hold nothing. If most individual developers have a subscription and no API account, requiring a key at sign-up excludes them at the door. **The conditions, verbatim from `code.claude.com/docs/en/legal-and-compliance` (2026-09-03):** an **unmodified** binary, no authentication method removed, disabled or restricted (including sign-in with a Claude account), no paying/reselling/intermediating usage — each end user authenticates with their own credential and is billed directly — a **branding restriction** (plain text may say the product runs Claude Code; the name and logo may not enter ours), and **accepting the Commercial Terms of Service**, which the previous note omitted. A commercial caveat, not a legal one: *"advertised usage limits for Pro and Max plans assume ordinary, individual usage"* — an agent working for hours is not obviously that, and the user hitting the ceiling will look like our fault. Architectural consequence, unchanged: the **tool loop becomes Claude Code's**, not ours | **P-23**; the order is the owner's call |
