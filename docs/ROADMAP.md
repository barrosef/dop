...
| P-30 | **O convite não leva link de aceite** — o token é devolvido uma vez e guardado com hash, e NÃO está no payload de `dop.identity.invite.created`. O gatilho só enxerga o evento, então o e-mail anuncia o convite e aponta para `/convites`, sem link direto. Resolver exige decisão: pôr o token no evento (credencial em repouso no log, replicada para toda projeção que assina `dop.>` — não recomendo) ou um caminho lateral que o leve ao notificador fora do log | dop-core; ADR-0025 |
