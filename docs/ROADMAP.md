...
| P-16 | **Decided on 2026-09-03**, and it changed shape. NOT a fourth layer in the context package: a project's documents are **indexed** and their storage is **mounted as a volume** in that project's runners, so the agent reads them from the filesystem with `run_command` — the way the single-tool design already intends. **The mechanism is DONE (2026-09-03):** the documents travel in the `SandboxSpec`, each substrate materializes them natively (a read-only ConfigMap on k8s, an archive owned by root on Docker), and four new guarantees (18 to 21) prove it in BOTH adapters. What is left is **which artefacts are documents**: the knowledge domain has three kinds — rule, index, memory — and neither `spec` nor `plan` is among them (ADR-0009 §1 fixes the three). Filling the mount is one port method away; deciding what goes in it touches the ADR | ADR-0009; the mechanism ✅, the filling open |
